...
// El NOPAL tambien se dibuja como sprite, pero con GROSOR: en vez de dos
...
bool esSueloParaNopal(BlockType type) {
    switch (type) {
        ...
        // Pasto, tierra, arena, grava, arcilla, tierra/arena arcillosa...
        default:
            return true;
    }
}
...
struct NopalForma {
    // Conexiones con los SEIS vecinos. Un cladodio puede unirse a los seis a
    // la vez, y de esa combinacion sale su forma.
    bool uneXp, uneXm;       // este / oeste
    bool uneZp, uneZm;       // sur / norte
    bool uneArriba, uneAbajo;

    // Caja del bloque cuando NO hay continuacion por ese lado. Segun la
    // orientacion, la penca es una losa vertical (fina en X o en Z), una losa
    // horizontal (fina en Y) o un bloque achatado.
    float minX, maxX, minY, maxY, minZ, maxZ;

    bool ejeX;               // compatibilidad: la penca se alarga en X
    bool unePlus, uneMenos;  // compatibilidad: continua por el eje largo
    int  patron;             // 0..99
    int  orientacion;        // 0..5
    int  forma;              // 0..599
...
constexpr int NOPAL_FORMAS        = NOPAL_PATRONES * NOPAL_ORIENTACIONES;

// Calcula la forma a partir de los vecinos REALES. `get(dx,dy,dz)` devuelve
// el BlockType vecino.
//
// La ORIENTACION decide en que eje es fina la penca:
//   0,1 -> fina en Z  : losa VERTICAL mirando al norte/sur
//   2,3 -> fina en X  : losa VERTICAL mirando al este/oeste
//   4   -> fina en Y  : losa HORIZONTAL (una penca tumbada)
//   5   -> achatada   : algo mas gruesa en los tres ejes
...
NopalForma calcularFormaNopalCon(TGet get, int wx, int wy, int wz,
    ...
    // ========================================================================
    // MEDIDAS DE LA PENCA: 15 de largo x 8 de ancho x 13 de grosor
    // ========================================================================
    ...
    // estrecha, que en la rejilla del voxel casi no se veria.
    //
    // Aqui la pieza queda MAS GRUESA QUE ANCHA (13 contra 8), justo al reves
    // que en la planta, donde el cladodio es catorce veces mas ancho que
    // grueso. Es intencionado: asi la penca se ve maciza en el voxel.
    //
    ...
    f.uneMenos = f.ejeX ? f.uneXm : f.uneZm;

    // --- PATRON (0..99) ---
    // 64 combinaciones de vecindad + 36 variantes de crecimiento.
    const int bits = (f.uneXp ? 1 : 0) | (f.uneXm ? 2 : 0) |
                     (f.uneZp ? 4 : 0) | (f.uneZm ? 8 : 0) |
                     (f.uneArriba ? 16 : 0) | (f.uneAbajo ? 32 : 0);
    const int variante = (int)((h >> 4) % 36u);
    f.patron = bits;
    if (bits >= 28) f.patron = 64 + variante;
...
bool nopalHitboxCon(BlockType type, TGet get, int wx, int wy, int wz,
    ...
    // La TUNA ya no forma parte de esta caja: es un bloque aparte, con la
    // suya. Asi el jugador la selecciona y la arranca sin tocar la penca, y
    // la penca conserva su forma limpia.
    return true;
}
...
public:
    ...
    void loadAllBlockTextures() {
        ...
        loadTexture("Tallo de Nopal de Castilla en tierra arcillosa.png");
        loadTexture("Tallo de Nopal de Castilla en arena arcillosa.png");
...
public:
    ...
    void generateChunk(Chunk* chunk) {
        ...
        for (int x = 0; x < CHUNK_SIZE; x++) {
            for (int z = 0; z < CHUNK_SIZE; z++) {
                ...
                // ---- NOPAL DE CASTILLA ----
                // Crece en CUALQUIER suelo natural blando: pasto, tierra,
                // arena, grava, arcilla y sus variantes. NO en piedra, caliza
                // ni minerales: un nopal no arraiga en roca desnuda.
                //
                // Es raro (1 de cada ~700 columnas validas) para que
                // encontrarlo tenga valor, y se separa de los arboles para
                // que no salga uno dentro de la copa.
                if (esSueloParaNopal(ground) &&
                    chunk->getBlock(x, surfaceY + 1, z) == BLOCK_AIR) {
                    const int hn = worldX * 4517 + worldZ * 8291;
                    unsigned v = (unsigned)(hn ^ 0x9E3779B9u);
    ...
    //   - BASE: un bloque a ras de suelo, con la textura que se funde con el
    //     terreno sobre el que crece.
    //   - TALLO: 3 a 6 bloques de alto encima de la base.
    //   - CLADODIOS: las pencas ovaladas. Brotan a los lados del tallo y
    //     miden de 1 a 3 bloques. Cada cladodio puede a su vez sacar otro,
    //     que es como se ramifica un nopal de verdad.
    //
    ...
    void generarNopal(int worldX, int baseY, int worldZ) {
        ...
        for (int i = 1; i <= alturaTallo; ++i) {
            ...
            if (getBlock(worldX, y, worldZ) != BLOCK_AIR) break;
            setBlock(worldX, y, worldZ, BLOCK_NOPAL_TALLO);
            topeTallo = y;
        }
                ...
                for (int y = 0; y < CHUNK_HEIGHT; y++) {
                    ...
                    if (isCrossSprite(block)) {
                        ...
                        if (esCladodio(block) ||
                            ...
                            // ============================================
                            // NO es una cruz como la hierba: la penca es un
                            // volumen. Se dibuja como una CAJA con sus SEIS
                            // caras, cada una emitida por separado y con su
                            // normal correcta.
                            //
                            // Antes las caras salian de pushQuad(), que emite
                            // un quad vertical A DOBLE CARA y siempre de la
                            // base al techo del voxel. Eso tenia dos
                            // consecuencias malas: la caja no tenia tapa ni
                            // fondo (se veia hueca por arriba) y las caras
                            // laterales compartian vertices con su gemela
                            // invertida, de modo que segun el angulo una de
                            // ellas ganaba el z-fight y la otra desaparecia:
                            // ese era el bug de "la cara norte sin textura".
                            //
                            // Ahora cada cara es un quad UNICO con su
                            // orientacion, y la caja se recorta contra los
                            // SEIS vecinos: crece hasta el borde por el lado
                            // donde la planta continua, y la cara interior de
                            // esa union no se dibuja. De ahi salen las formas
                            // (losa vertical, losa horizontal, bloque
                            // achatado, L, T...) sin codigo especial para
                            // cada una: son consecuencia de que vecinos hay.
                            ...
                            // ============================================
                            // LA TUNA: BLOQUE PROPIO
                            // ============================================
                            // Ya no se dibuja aqui dentro: es un bloque con su
                            // ID (BLOCK_TUNA), su caja y su dureza, que crece
...
void drawItemIcon(BlockType blockType, float cx, float cy, float size) {
    ...
    // cual, en 2D, aunque SI sea colocable.
